...
sol1{1} = ode45(M1,[t1{1}(1),t1{1}(end)],[initConds],'refine',10);     %Solve it
...
for i = 2:N*2
    ...
    sol1{i} = ode45(M1,[t1{i}(1),t1{i}(end)],[deval(sol1{i-1},t1{i-1}(end))],'refine',10);     
end
    

[V2] = odeToVectorField(diff(diff(y)) + W0/Q*diff(y) + W0^2*y == 0);
M2 = matlabFunction(V1,'vars', {'t','Y'});                   
sol2 = ode45(M2,[t2(1),t2(end)],[deval(sol1{N*2},t1{N*2}(end))],'refine',10);  
